...
// Given an array of college graduates.  Complete the following requests WITHOUT using any
//array methods like .forEach(), .map(), .reduce(), .filter()
...
let uniArray = graduates.map(function(uniYo) {
  return uniYo.university;
});
...
const contactInfo = [];
graduates.forEach(function(contactsYo) {
  contactInfo.push(contactsYo.first_name);
  contactInfo.push(contactsYo.email);
});
...
const uniArr = graduates.map(function(uniArrYo) {
  return uniArrYo.university;
});

const uniArrAlph = uniArr.filter(function(uniArrAlphYo) {
  if (uniArrAlphYo.includes("Uni")) {
    return uniArrAlphYo.university;
  }
});

console.log(uniArrAlph);

// const uniArr = graduates.filter(function(uniArrYo) {
//   if(uniArrYo.includes("Uni")) {
//     return uniArrYo.university;
//   }
// });
...
*/
const animalNames = [];
zooAnimals.forEach(function(zooAnimalsYo) {
  animalNames.push(zooAnimalsYo.animal_name);
  animalNames.push(zooAnimalsYo.scientific_name);
});
...
const lowerCase = zooAnimals.map(function(lowerCaseYo) {
  return lowerCaseYo.animal_name.toLowerCase();
});
...
const largerPopulation = zooAnimals.filter(function(largerPopulationYo) {
  if (largerPopulationYo.population < 5) {
    return largerPopulationYo.population;
  }
});
...
const populationTotal = zooAnimals.reduce(function(
  accumulator,
  populationTotalYo
) {
  return accumulator + populationTotalYo.population;
},
0);
